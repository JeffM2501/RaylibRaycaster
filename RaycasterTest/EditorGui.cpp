...
void GuiTest()
{
	...
	sliderValue = (int)GuiSlider(Rectangle{ 355, 400, 165, 20 }, "TEST", TextFormat("%2.2f", (float)sliderValue), (float)sliderValue, -50, 100);
	sliderBarValue = (int)GuiSliderBar(Rectangle{ 320, 430, 200, 20 }, NULL, TextFormat("%i", (int)sliderBarValue), (float)sliderBarValue, 0, 100);
	...
	if (showMessageBox)
	{
		...
		int result = GuiMessageBox(Rectangle{ (float)GetScreenWidth() / 2 - 125, (float)GetScreenHeight() / 2 - 50, 250, 100 }, GuiIconText(RICON_EXIT, "Close Window"), "Do you really want to exit?", "Yes;No");
	...
	{
		DrawRectangle(0, 0, GetScreenWidth(), GetScreenHeight(), Fade(RAYWHITE, 0.8f));
		int result = GuiTextInputBox(Rectangle{ (float)GetScreenWidth() / 2 - 120, (float)GetScreenHeight() / 2 - 60, 240, 140 }, GuiIconText(RICON_FILE_SAVE, "Save file as..."), "Introduce a save file name", "Ok;Cancel", textInput);
